...
infile = open(r'C:\git_vscode\edu_file','r')
text = infile.readlines()
word = random.choice(text).rstrip()
answer = list(word)
answer_ = list('_' * len(answer))
trial = 10

for c in range(trial+1):
    n = 0
    user_answer = input('단어를 추측하세요:')
    for i in word:
    ...
    if answer_ == answer:
        print('성공 :D')  
        break
    elif c == trial and answer_ != answer:
        print('실패 :<')
        break
